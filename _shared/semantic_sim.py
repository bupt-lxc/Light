# -*- coding: utf-8 -*-
"""共享语义相似度 (semantic_sim) — Light v2 / _shared 地基契约 3。

目的
----
消灭"该用语义的地方全用词面 Jaccard"的全包瘟疫。同一缺口在多个技能里各犯一遍
= 多次代差。本契约一次建设、多处复用。

最臭名昭著的反例:用字符 Jaccard 判标题撞车,"Attention Is All You Need"
...
诚实边界(对标 RapidFuzz vs sentence-transformers 实测):离线档是**字面/词形**匹配,
**做不了纯语义同义**("running shoes"↔"athletic footwear" 这类无共词同义,离线档必然
低分)。需要可靠语义时注入 embedding 档;离线档不假装能做,选 mode 时按需升档。

被谁消费(v2)
-------------
literature-search(相关度重排) / citation(标题·作者匹配) / idea-generation(中文贡献去重) /
idea-critique(撞车检测) / venue-matching(稿件→venue 匹配) / consistency(贡献表述漂移)
...
def _offline_similarity(a: str, b: str) -> float:
    ...
    tok_jac = len(sta & stb) / len(sta | stb) if (sta | stb) else 0.0  # 词重合(词序无关)
...
def _selftest() -> int:
    ...
    # 2. 倒装句:高相似但 <1.0(核心测试 —— 治裸 Jaccard 的 1.0 误判)
    ...
    # 4b. 派生词/纯同义词(classification↔classifying)是离线档边界,
