...
# -*- coding: utf-8 -*-
"""
gate_runner.py —— gate 聚合执行器 (Light v2 / _shared 地基契约 2)

把多个独立 gate 函数(证据↔措辞、引用支撑、draft-lint、结论夸大门 ...)跑成一份
统一的 light.findings.v1 报告。任一 critical fail -> 整体 verdict=fail。
这是**总控确认点判定的接线处**:总控不消费 prose verdict,改读本报告的 verdict 字段。
...
用法:
  ...
  消费方 import:
      ...
      report = run_gates([gate_a, gate_b], artifact,
                         producer="orchestrator", target="draft.md")
...
# 同目录 import:既支持作为脚本直接跑,也支持作为 _shared 包模块被 import
try:
    from findings_schema import Finding, GateResult, FindingsReport
except ImportError:  # 作为包 _shared 被导入时
...
def _selftest() -> int:
    ...
    check(rep.producer == "orchestrator" and rep.target == "draft.md",
          "producer/target 应透传")
